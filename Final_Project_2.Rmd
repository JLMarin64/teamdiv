...
Taking account for all the tests and factors used in comparing the two models, it is determined that the intuition model is the final model chosen. Although the fit statistics is slightly lower than the base model, however there is lower likelihood of multicollinearity with the substituted variable and the Hosmer-Lemmeshow shows better fit than the base model. Also factors seem to explanatory power in differentiating between benign and malignant. Therefore, the final model equation chosen has the following factors Bare.nuclei, Bl.cromatin, Cell.Shape, and Marg.adhesion.
...
Upon doing the Logistic Regression model for the Breast Cancer data set, we would like to evaluate other models such as Random Forect and Decision Trees to see if we can improve the predictability of the model. 
...
A random forest model was laso created and the feature importance was re-evaluated with the random forest algorithm.  
...
```
# Conclusion
Though the accuracy the Random Forest model is higher at almost 97%, at this point in time, it may be better to use the Logistic Regression model given that it is more important to predict malignant cases.  
